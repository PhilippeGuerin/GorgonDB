...
--
-- Name: activeserver(text); Type: FUNCTION; Schema: gorgon; Owner: operator
--

CREATE FUNCTION activeserver(p_server_name text) RETURNS integer
    LANGUAGE plpgsql
    AS $$
declare
  p_channel text := 'gorgon';
  begin
  update servers set active = FALSE;
  update servers set active = TRUE where lower(server_name) like lower(p_server_name);
...
ALTER FUNCTION gorgon.activeserver(p_server_name text) OWNER TO operator;
...
--
-- Name: process_server_schema(integer, text); Type: FUNCTION; Schema: gorgon; Owner: operator
--

CREATE FUNCTION process_server_schema(p_server_id integer, p_schema_name text) RETURNS integer
    LANGUAGE plpgsql
    AS $_$ 
declare
  v_connection_string text;
...
begin
  ...
  select connection_string, server_name into v_connection_string, v_server_name from servers where server_id = p_server_id;
  perform pg_notify(p_channel, 'Cleanup previous data for server ' || v_server_name);
  raise notice 'Cleanup previous data for serve:%',p_schema_name ;
  --perform cleanup_server(p_server_id);
...
begin
    ...
    select name from (select '''' || name ||'''' as name from gorgon.excluded_schema offset 0 limit 1) req union all select name from (select ',''' || name ||'''' as name from gorgon.excluded_schema offset 1) req into v_excluded_schemas;  
    select public.dblink_open(v_server_name, 'cast_schemas', 'select schemaname from pg_tables where schemaname like '''||p_schema_name||'%'' and tablename = ''sys_package_version'' and (schemaname not like ''%schemacopy%'' and schemaname not like ''%test%'' and schemaname not like ''%OLD'' and schemaname not in ('||v_excluded_schemas||')) order by 1') into v_dblink_retval; 
    exception when OTHERS then
      raise notice ' EXCEPTION                         $$' ;
      perform pg_notify(p_channel, 'Exception (process_server (id)): ' || SQLERRM || '(' || SQLSTATE || ')');
    end;
...
ALTER FUNCTION gorgon.process_server_schema(p_server_id integer, p_schema_name text) OWNER TO operator;
